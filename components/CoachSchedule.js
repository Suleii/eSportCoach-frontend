...
function CoachSchedule(props) {
  ...
  useEffect(() => {
    ...
    let datesdisabled = disabledDays.some((day) =>
      isSameDay(new Date(day.date), new Date(selectedDate))
    );
                ...
                bookingsSelected.map((booking, i) => {
                  return (
                    <Event
                      key={i}
                      game={booking.game}
                      partner={booking.username.user.username}
                      date={booking.date}
                      coach={booking.coachUsername}
                      id={booking._id}
                    />
                  );
                })
          ...
          <div className="flex flex-col justify-center w-full items-center">
            <div className="bg-base-100 rounded-xl">
              <DayPicker
                ...
                modifiersStyles={{
                  disabledDates: disabledStyles,
                  bookingsDates: bookingsStyles,
                }}
                modifiersClassNames={{
