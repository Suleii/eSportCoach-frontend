...
const personalStars = [];
      for (let i = 0; i < 5; i++) {
        ...
        personalStars.push(<FontAwesomeIcon key={i} icon={faStar} onClick={() => setNote(i + 1)} style={style} className="note" />);
      }
        ...
        <div className="flex justify-end mb-6" onClick={handleClose}>
          ...
          <label className="form-control ">
            <div className="label flex">
                <span className="text-lg text-white">Your review for @{selectedCoach}</span>
                <div> {personalStars}</div>
            </div>
            
            <textarea minLength="10" maxLength="120" required placeholder="Tell us what you think" onChange={handleInputChange} className="textarea bg-neutral textarea-bordered textarea-lg text-primary h-56" ></textarea>
        </label>  
        ...
        ?   ( <div role="alert" className=" flex flex-row justify-between w-full">
                <div className='flex flex-col w-full'>
                    ...
                    <div  className="flex flex-row items-center justify-between mb-2">
                        ...
                        :(<p>No review for this coach yet.&nbsp; <span className= "underline underline-offset-4 decoration-info" onClick={openModal}>Do it now!</span></p>)
                        }
