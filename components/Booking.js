...
function Booking(props) {
  ...
  const gamesList = games.map((data, i) => (
    <option className="hover:bg-base-100 focus:bg-base-100 w-100" value={data}>
      {data}
    </option>
  ));
  ...
  const availableTimes = times.filter(
    (time) =>
      !bookings.some((bookingDate) =>
        isEqual(new Date(time), new Date(bookingDate))
      )
  );
    ...
    <div className="flex flex-col items-center ">
      <div className=" flex flex-col w-5/6 flex-1">
        ...
        <div className="flex flex-col justify-center w-full items-center">
          ...
          >
            <option className="btn m-1 " disabled value="question">
              Please choose your game:
            </option>
            {games.length > 0 ? (
              gamesList
            ) : (
              <option className="btn m-1" value="" disabled>
                No games for this coach
              </option>
            )}
          </select>
          ...
          <div className="grid grid-cols-3 gap-x-6 gap-y-2 mt-5 mb-10">
            {availableTimes.map((time, i) => {
              return (
                <div
                  key={`time-${i}`}
                  className="rounded mt-2 h-8 w-20 text-sm border border-accent border-1.25 bg-primary hover:bg-accent text-white flex justify-center "
                >
                  <button
                    type="button"
                    onClick={() => {
        ...
        <button
          type="button"
          className="w-80 mx-auto btn btn-success text-white"
